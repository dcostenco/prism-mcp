...
app = modal.App("prism-v18coder-14b-sft")
...
image = (
    ...
    .pip_install(
        ...
        "trl==0.18.0",            # compat with transformers 4.55
        "peft==0.15.0",           # compat with transformers 4.55
        "datasets==3.6.0",
        "accelerate==1.7.0",
        "bitsandbytes==0.45.3",
        "huggingface_hub",
...
OUT_VOL = modal.Volume.from_name("prism-v18coder-14b", create_if_missing=True)
...
@app.function(
    image=image,
    gpu="H100:2",  # 4×H100 for ~4× throughput on 190K rows
    timeout=10 * 60 * 60,
...
def run_full_sft():
    ...
    print(f"=== v18coder-14B FULL SFT (r=384, full 190K) starting ===")
    print(f"  base = {BASE_MODEL}  (Qwen3-8B — best 7-9B BFCL V4 base)")
    ...
    # Add random import for reproducible subsample
    import random
    ...
    # Subsample to 60K rows for first run (full 190K too slow)
    if len(rows) > 60000:
        random_seed_ds = random.Random(42)
        rows_sub = random_seed_ds.sample(rows, 60000)
        ds = Dataset.from_list(rows_sub)
        print(f"subsampled to {len(rows_sub)} rows for tractable first pass")
    cfg = SFTConfig(
        output_dir="/out/v18coder_14b_run",
        num_train_epochs=1,                     # 2 epochs (was 3)
        per_device_train_batch_size=4,          # bigger batch (was 2) — Qwen3-8B fits on H100 80GB
        gradient_accumulation_steps=2,          # no accum
    ...
    print(f"v18coder-14B full SFT done in {train_secs:.0f}s")
    ...
    Path("/out/v18coder_14b_meta.json").write_text(json.dumps(meta, indent=2))
    OUT_VOL.commit()
    return meta
...
def run():
    print("Launching v18coder-14B (r=384, full 190K) full SFT on H100…")
    meta = run_full_sft.remote()
    print("\n" + "=" * 60)
    print("v18coder-14B FULL SFT COMPLETE")
    print("=" * 60)

...
Strategy:
  ...
  - SFT on combined train_v16_3.jsonl: v16.1 corpus (9,939 rows) + ~5K BFCL rows
...
Local launch (after data is ready):
  ...
  python3 merge_v16_data.py --out data/train_v16_3.jsonl
  modal volume put prism-sft-data data/train_v16_3.jsonl /train_v16_3.jsonl --force
  modal run --detach modal_v16_3_sft.py::train

Fetch adapter when done:
  modal run modal_v16_3_sft.py::fetch
"""
import json
import os
...
# Volumes:
#   prism-sft-data   - holds train_v16_3.jsonl (uploaded by client)
#   prism-v16-3      - holds the trained adapter + gate report
data_vol = modal.Volume.from_name("prism-sft-data", create_if_missing=True)
out_vol = modal.Volume.from_name("prism-v16-3", create_if_missing=True)
...
def run_sft_with_gate():
    ...
    train_path = Path("/data/train_v16_3.jsonl")
    if not train_path.exists():
        raise FileNotFoundError(
            f"{train_path} missing — upload first with:\n"
            "  modal volume put prism-sft-data train_v16_3.jsonl /train_v16_3.jsonl --force"
        )
    rows = []
    ...
    Path("/out/v16_3_gate.json").write_text(json.dumps(gates, indent=2))
    out_vol.commit()
...
def upload_data(local_path: str = "/Users/admin/prism/training/data/train_v16_3.jsonl"):
    """Upload merged training file to volume."""
    import subprocess
    p = Path(local_path)
    if not p.exists():
        print(f"ERROR: {p} not found. Run merge_v16_data.py --out data/train_v16_3.jsonl first.")
        return
    print(f"Uploading {p} ({p.stat().st_size/1e6:.1f} MB) to prism-sft-data:/train_v16_3.jsonl")
    subprocess.run(
        ["modal", "volume", "put", "prism-sft-data", str(p), "/train_v16_3.jsonl", "--force"],
        check=True,
    )
...
@app.local_entrypoint()
def fetch(out_dir: str = "/Users/admin/prism/training/models/v16_3_modal"):
    """Pull v16.2 adapter from Modal Volume."""
    import subprocess
    os.makedirs(out_dir, exist_ok=True)
    print(f"fetching v16_3_adapter to {out_dir}...")
    subprocess.run(
        ["modal", "volume", "get", "prism-v16-3", "v16_3_adapter", out_dir, "--force"],
        check=True,
    )
    subprocess.run(
        ["modal", "volume", "get", "prism-v16-3", "v16_3_gate.json", f"{out_dir}/v16_3_gate.json", "--force"],
        check=False,
    )
...
if __name__ == "__main__":
    print("Run via: modal run --detach modal_v16_3_sft.py::train")

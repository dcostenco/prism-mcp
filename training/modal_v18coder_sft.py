...
app = modal.App("prism-v18coder-sft")
...
OUT_VOL = modal.Volume.from_name("prism-v18coder", create_if_missing=True)
...
def run_full_sft():
    ...
    print(f"=== v18coder FULL SFT starting ===")
    ...
    cfg = SFTConfig(
        output_dir="/out/v18coder_run",
        num_train_epochs=1,                     # 2 epochs (was 3)
        per_device_train_batch_size=8,          # bigger batch (was 2) — Qwen3-8B fits on H100 80GB
        gradient_accumulation_steps=1,          # no accum
    ...
    print(f"v18coder full SFT done in {train_secs:.0f}s")
    ...
    Path("/out/v18coder_meta.json").write_text(json.dumps(meta, indent=2))
    OUT_VOL.commit()
    return meta
...
def run():
    print("Launching v18coder full SFT on H100…")
    meta = run_full_sft.remote()
    print("\n" + "=" * 60)
    print("v18coder FULL SFT COMPLETE")
    print("=" * 60)

# BFCL V4 Training Pipeline — Round 12 Adversarial Review Prompt
...
- **SFT Phase 1a**: `generate_bfcl_training_data.py` — BFCL V4 data with CoT, multi-step scenarios extract primary tool from `steps[0]`.
- **SFT Phase 1b**: `generate_diverse_sft.py` — supplemental SFT with system prompts via `format_system_prompt()`. Exp 3 combines `_sys_prompt + state`. `generate_sft_toolnames.py` — tool name disambiguation.
- **GRPO Phase 2**: `bfcl_grpo_align.py` — reads `{"messages": [...]}`, captures `user|assistant|tool` roles.
- **Continuous Learning**: `continuous_learning.py` — preference signals from SQLite with system prompt + loaded tool schemas.
- **Schema Registry**: `build_tool_schema.py` — exports Prism tools + V4 Agentic tools (from `V4_API_SCHEMAS`) to `tool_schema.json`.
- **Pipeline**: `run_bfcl_pipeline.sh` — runs `build_tool_schema.py` → `semantic_rag.py hyde` → training → eval.
- **Evaluation**: `bfcl_eval.py` — Ollama `raw:true` + ChatML, JSON array guards, multi-turn followup evaluation. `benchmark.py` — `mlx_lm.generate` with full schemas, escaped regex. `swe_bench_test.py` — Ollama `raw:true` + ChatML system prompt.
- **RAG**: `semantic_rag.py` — HyDE-based retrieval from `tool_schema.json`.

## Cumulative Fix History (R7-R11) — DO NOT RE-REPORT
...
| R9 | 14 | Parallel tool call stuttering | `generate_bfcl_training_data.py` |
...
## Cumulative Checklist (R1-R11) — ALL PASSING
...
Review the attached repomix for Round 12 bugs. Focus on:

1. **Complete end-to-end trace**: Pick one training example type (e.g., multi-turn) and trace it from raw JSON → SFT JSONL → GRPO alignment → evaluation. Verify format parity at every boundary.
2. **System prompt identity**: Is `format_system_prompt()` called with identical tool lists across ALL consumers? Any drift?
3. **Token consistency**: Are `<|synalux_think|>`, `<|tool_call|>`, `<|tool_response|>`, `<|im_start|>` used identically in generators, parsers, and evaluators?
4. **Edge case robustness**: Empty strings, unicode, partial JSON, nested arrays, extremely long outputs.
5. **Hyperparameter sanity**: LoRA rank, learning rate, batch size, temperature, max_seq_length for target model sizes.
6. **Security**: Any injection vectors through user prompts or tool responses?
7. **Performance**: Any O(n²) loops, redundant file reads, or memory leaks in training/eval?

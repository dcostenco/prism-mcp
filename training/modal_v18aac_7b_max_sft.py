...
app = modal.App("prism-v18aac-7b-max-sft")
...
OUT_VOL = modal.Volume.from_name("prism-v18aac-7b-max", create_if_missing=True)
...
def run_full_sft():
    ...
    print(f"=== v18aac FULL SFT starting ===")
    print(f"  base = {BASE_MODEL}  (Qwen2.5-Coder-7B-Instruct (proven base for AAC))")
    ...
    # DoRA configuration — MAX precision: rank 512, alpha 1024
    lora_cfg = LoraConfig(
        r=512, lora_alpha=1024,
    ...
    cfg = SFTConfig(
        output_dir="/out/v18aac_7b_max_run",
        num_train_epochs=5,
    ...
    print(f"v18aac full SFT done in {train_secs:.0f}s")
    ...
    meta = {
        "train_secs": round(train_secs, 1), "epochs": 5,
        "lr": cfg.learning_rate, "examples": len(rows),
        "lora_rank": 512, "lora_alpha": 1024, "use_dora": True,
        "base_model": BASE_MODEL,
        "gpus": "H100:4",
        "data_composition": "v18aac AAC dataset (caregiver, emergency, text_correct, ask_ai, format_anchor, gestures)",
        "checkpoints": [f"/out/epoch_{i}_adapter" for i in range(1, 6)] + ["/out/final_adapter"],
    }
    Path("/out/v18aac_meta.json").write_text(json.dumps(meta, indent=2))
    OUT_VOL.commit()
    return meta
...
def run():
    print("Launching v18aac full SFT on H100…")
    meta = run_full_sft.remote()
    print("\n" + "=" * 60)
    print("v18aac FULL SFT COMPLETE")
    print("=" * 60)

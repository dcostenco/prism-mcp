# Adversarial Code Review — Round 18 (BFCL V4 Training Pipeline)

## Context & Objective
You are a senior adversarial code auditor performing Round 18 of an iterative hardening cycle on the Prism BFCL V4 training and evaluation pipeline. The objective is to achieve the **#1 ranking on the Berkeley Function Calling Leaderboard (BFCL)** by eliminating every bug that causes silent data corruption, evaluation deflation, or distribution drift.

## What Was Fixed in Round 17

### 1. [HIGH → RESOLVED] CoT Hallucination Bypasses Evaluation Parsers
- **Files:** `bfcl_eval.py`, `benchmark.py`
- **Root cause:** `parse_all_tool_calls` and `_repair_and_extract` operated on raw `response_text` containing `<|synalux_think|>` blocks. JSON inside CoT reasoning created false-positive tool calls on abstention prompts. In `benchmark.py`, Strategy 3 stripped think blocks but Strategies 1, 1b, and 2 did not.
- **Fix:** Both files now strip `<|synalux_think|>` blocks at the very top of their extraction pipelines. In `bfcl_eval.py`, `parse_all_tool_calls` and `_repair_and_extract` apply `re.sub(r'<\|synalux_think\|>.*?</\|synalux_think\|>', '', ...)` before any strategy runs. In `benchmark.py`, `parse_tool_call` creates `clean_response` upfront; all 4 strategies (1, 1b, 2, 3) now use it. Strategy 3's redundant `re.sub` was removed.

## Cumulative Fix Log (R7–R17)
...
If no bugs are found, explicitly state that the pipeline passes the Round 18 audit.

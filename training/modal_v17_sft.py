"""Modal H100 v17 SFT — adds tone-classification + boosted irrelevance + caregiver.

Strategy:
  - Start from clean Qwen2.5-Coder-7B-Instruct (BF16, transformers-loadable)
  - SFT on combined train_v17.jsonl: v16.3 corpus (~12K rows) + ~5.5K v17 deltas
    (tone_switch ~2K, caregiver_parse_extra boosted ~1.5K, bfcl_irrelevance
    boosted ~2K)
  ...
  - IN-CONTAINER RELIABILITY GATE — adapter saved only if ALL pass:
      * BFCL probe >= 80% (vs 78% in v16.x — push irrelevance)
      * AAC text_correct held-out >= 89%
      * Emergency Q&A held-out = 5/5 (HARD — proxy for full 13/13)
      * Caregiver held-out >= 85% (fix v16.3's 71.4% regression)
      * Tone-switch held-out >= 80% (NEW gate — new feature must work)
...
Local launch (after data is ready):
  ...
  python3 merge_v16_data.py --out data/train_v17.jsonl
  modal volume put prism-sft-data data/train_v17.jsonl /train_v17.jsonl --force
  modal run --detach modal_v17_sft.py::train

Fetch adapter when done:
  modal run modal_v17_sft.py::fetch
"""
import json
import os
...
# Volumes:
#   prism-sft-data   - holds train_v17.jsonl (uploaded by client)
#   prism-v17      - holds the trained adapter + gate report
data_vol = modal.Volume.from_name("prism-sft-data", create_if_missing=True)
out_vol = modal.Volume.from_name("prism-v17", create_if_missing=True)
...
def run_sft_with_gate():
    ...
    print(f"=== v17 SFT starting; base = {BASE_MODEL} ===")
    ...
    train_path = Path("/data/train_v17.jsonl")
    if not train_path.exists():
        raise FileNotFoundError(
            f"{train_path} missing — upload first with:\n"
            "  modal volume put prism-sft-data train_v17.jsonl /train_v17.jsonl --force"
        )
    rows = []
    ...
    cfg = SFTConfig(
        output_dir="/out/v17_run",
        num_train_epochs=1,
        # 3000 steps × batch 2 × grad_accum 8 = 48,000 example-forwards over
        # 13,745 rows ≈ 3.5 epochs. Sweet spot for SFT quality on this size
        # corpus before overfitting risk increases. Cosine LR scheduler scales
        # with max_steps so peak LR remains 2e-5 with 3% warmup.
        max_steps=3000,
    ...
    Path("/out/v17_gate.json").write_text(json.dumps(gates, indent=2))
    out_vol.commit()
...
# v17 — caregiver note parsing held-out (target ≥85% to fix v16.3's 71.4%)
_CAREGIVER_HELDOUT = [
    {
...
# v17 — tone-switch held-out. Each case: input + acceptable tones (any match counts).
_TONE_HELDOUT = [
...
def _run_reliability_gate(model, tok):
    ...
    # 4) Caregiver note parsing — must be ≥85%
    cg_passes = 0
    ...
    if cg_pct < 0.85:
        failures.append(f"caregiver {cg_pct*100:.1f}% < 85%")

    # 5) Tone-switch — must be ≥80% (NEW v17 gate)
    import re
    tone_passes = 0
...
def upload_data(local_path: str = "/Users/admin/prism/training/data/train_v17.jsonl"):
    """Upload merged training file to volume."""
    import subprocess
    p = Path(local_path)
    if not p.exists():
        print(f"ERROR: {p} not found. Run merge_v16_data.py --out data/train_v17.jsonl first.")
        return
    print(f"Uploading {p} ({p.stat().st_size/1e6:.1f} MB) to prism-sft-data:/train_v17.jsonl")
    subprocess.run(
        ["modal", "volume", "put", "prism-sft-data", str(p), "/train_v17.jsonl", "--force"],
        check=True,
    )
...
@app.local_entrypoint()
def train():
    """Submit v17 SFT job to Modal H100. Run upload_data first."""
    print("Submitting v17 SFT to Modal H100...")
    result = run_sft_with_gate.remote()
    print(json.dumps(result, indent=2)[:3000])


@app.local_entrypoint()
def fetch(out_dir: str = "/Users/admin/prism/training/models/v17_modal"):
    """Pull v17 adapter from Modal Volume."""
    import subprocess
    os.makedirs(out_dir, exist_ok=True)
    print(f"fetching v17_adapter to {out_dir}...")
    subprocess.run(
        ["modal", "volume", "get", "prism-v17", "v17_adapter", out_dir, "--force"],
        check=True,
    )
    subprocess.run(
        ["modal", "volume", "get", "prism-v17", "v17_gate.json", f"{out_dir}/v17_gate.json", "--force"],
        check=False,
    )
...
if __name__ == "__main__":
    print("Run via: modal run --detach modal_v17_sft.py::train")

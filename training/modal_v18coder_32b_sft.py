...
app = modal.App("prism-v18coder-32b-sft")
...
OUT_VOL = modal.Volume.from_name("prism-v18coder-32b", create_if_missing=True)
...
@app.function(
    image=image,
    gpu="H100:4",  # 4×H100 for ~4× throughput on 190K rows
    timeout=10 * 60 * 60,
...
def run_full_sft():
    ...
    print(f"=== v18coder-32B FULL SFT starting ===")
    ...
    cfg = SFTConfig(
        output_dir="/out/v18coder_32b_run",
        num_train_epochs=1,                     # 2 epochs (was 3)
        per_device_train_batch_size=2,          # bigger batch (was 2) — Qwen3-8B fits on H100 80GB
        gradient_accumulation_steps=4,          # no accum
    ...
    print(f"v18coder-32B full SFT done in {train_secs:.0f}s")
    ...
    Path("/out/v18coder_32b_meta.json").write_text(json.dumps(meta, indent=2))
    OUT_VOL.commit()
    return meta
...
def run():
    print("Launching v18coder-32B full SFT on H100…")
    meta = run_full_sft.remote()
    print("\n" + "=" * 60)
    print("v18coder-32B FULL SFT COMPLETE")
    print("=" * 60)

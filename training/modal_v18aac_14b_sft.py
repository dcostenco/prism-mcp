...
app = modal.App("prism-v18aac-14b-sft")
...
OUT_VOL = modal.Volume.from_name("prism-v18aac-14b", create_if_missing=True)
...
def run_full_sft():
    ...
    print(f"=== v18aac-14B FULL SFT (r=384) starting ===")
    print(f"  base = {BASE_MODEL}  (Qwen2.5-Coder-14B-Instruct (proven base for AAC))")
    ...
    print(f"v18aac-14B full SFT done in {train_secs:.0f}s")
    ...
    Path("/out/v18aac_14b_meta.json").write_text(json.dumps(meta, indent=2))
    OUT_VOL.commit()
    return meta
...
def run():
    print("Launching v18aac-14B (r=384) full SFT on H100…")
    meta = run_full_sft.remote()
    print("\n" + "=" * 60)
    print("v18aac-14B FULL SFT COMPLETE")
    print("=" * 60)

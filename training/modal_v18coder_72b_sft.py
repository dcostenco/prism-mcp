...
app = modal.App("prism-v18coder-72b-sft")
...
OUT_VOL = modal.Volume.from_name("prism-v18coder-72b", create_if_missing=True)
...
@app.function(
    image=image,
    gpu="H100:8",  # 4×H100 for ~4× throughput on 190K rows
    timeout=10 * 60 * 60,
...
def run_full_sft():
    ...
    print(f"=== v18coder-72B FULL SFT starting ===")
    ...
    # MAX PRECISION: use full 190K dataset, no subsample
    cfg = SFTConfig(
        output_dir="/out/v18coder_72b_run",
        num_train_epochs=1,                     # 2 epochs (was 3)
        per_device_train_batch_size=1,          # bigger batch (was 2) — Qwen3-8B fits on H100 80GB
    ...
    print(f"v18coder-72B full SFT done in {train_secs:.0f}s")
    ...
    Path("/out/v18coder_72b_meta.json").write_text(json.dumps(meta, indent=2))
    OUT_VOL.commit()
    return meta
...
def run():
    print("Launching v18coder-72B full SFT on H100…")
    meta = run_full_sft.remote()
    print("\n" + "=" * 60)
    print("v18coder-72B FULL SFT COMPLETE")
    print("=" * 60)

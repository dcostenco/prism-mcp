"""v17.3 surgical pass — continue from v17.2, fix multi-turn + abstention.

v17.2 results:
  ✅ caregiver 6/7 + 20/20 targeted
  ✅ text_correct 15/15 (perfect)
  ✅ ask_ai 5/5, emergency_qa 12/13, translate 7/8
  ✅ BFCL 80.6%
  ❌ multi_turn 25% (was 87.5% in v17 — v18_1_multiturn data poisoned it)
  ❌ edge_case 62.5%
  ❌ joke abstention (calls search_web)

v17.3 approach: continue from v17.2 adapter, train short pass at LR 2e-6
on 3572 focused rows (mostly Prism-tool multi-turn + anti-hallucination +
abstention + holdovers). v18_1_multiturn DROPPED entirely.

Volumes:
  prism-v17-2      input  — /v17_2_adapter
  prism-sft-data   input  — /train_v17_4.jsonl
  prism-v17-4      output — /v17_4_adapter
"""
...
app = modal.App("prism-v17-4-surgical")
...
V17_2_VOL = modal.Volume.from_name("prism-v17-2")
OUT_VOL = modal.Volume.from_name("prism-v17-4", create_if_missing=True)
...
def run_surgical_sft():
    ...
    print(f"=== v17.3 surgical SFT starting ===")
    print(f"  base model = {BASE_MODEL}")
    print(f"  prior adapter = /v17_2/v17_2_adapter")
    ...
    model = PeftModel.from_pretrained(base, "/v17_2/v17_2_adapter", is_trainable=True)
    print("loaded v17.2 adapter")
    model.print_trainable_parameters()

    data_path = Path("/data/train_v17_4.jsonl")
    ...
    # 3572 rows × 2 epochs at eff_batch=8 → ~894 steps. Cap at 1000.
    cfg = SFTConfig(
        output_dir="/out/v17_4_run",
        num_train_epochs=2,
        max_steps=1000,
        per_device_train_batch_size=2,
        gradient_accumulation_steps=4,
        gradient_checkpointing=True,
        learning_rate=2e-6,                # LOWER again — third surgical pass, refine carefully
    ...
    Path("/out/v17_4_meta.json").write_text(json.dumps(meta, indent=2))
    OUT_VOL.commit()
    return meta
...
_SMOKE_PROBES = [
    ...
    },
    {
        ...
        "user": "Tell me a joke.",
        "expect_call": None,  # MUST abstain — this was failing in v17.1/v17.2
    },
    {
        "tools": None,
        "system": (
...
def run():
    print("Launching v17.3 surgical SFT on H100…")
    meta = run_surgical_sft.remote()
    print("\n" + "=" * 60)
    print("v17.3 SURGICAL SFT COMPLETE")
    print("=" * 60)
    print(json.dumps(meta, indent=2))
    print("\nFetch adapter:")
    print("  modal volume get prism-v17-4 v17_4_adapter ./v17_4_adapter --force")

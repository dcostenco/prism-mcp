...
- Some files may have been excluded based on .gitignore rules and Repomix's configuration
- Binary files are not included in this packed representation. Please refer to the Repository Structure section for a complete list of file paths, including binary files
- Only files matching these patterns are included: bfcl_qlora_finetune.py, bfcl_grpo_align.py, generate_bfcl_training_data.py, test_handler.py, run_bfcl_pipeline.sh
- Files matching patterns in .gitignore are excluded
- Files matching default ignore patterns are excluded
- Files are sorted by Git change count (files with more changes are at the bottom)
...
def generate_irrelevance_negatives(output_path: Path, collections: dict, num_examples: int = 2000):
    ...
    for i in range(num_examples):
        ...
        # Convert to raw text format matching handler's _format_prompt
        raw_text = format_as_raw_text(example["messages"], tools)
        examples.append({"text": raw_text, "category": "irrelevance"})
...
def generate_multiturn_examples(output_path: Path, collections: dict, num_examples: int = 1000):
    ...
    for i in range(num_examples):
        ...
        # Convert to raw text format matching handler's _format_prompt
        raw_text = format_as_raw_text(example_msgs, tools)
        examples.append({"text": raw_text, "category": "multi_turn"})
...
def generate_miss_func_examples(output_path: Path, collections: dict, num_examples: int = 500):
    ...
    for i in range(num_examples):
        ...
        raw_text = format_as_raw_text(msgs, tools)
        examples.append({"text": raw_text, "category": "miss_func"})
